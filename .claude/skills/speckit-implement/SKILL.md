...
1. Run `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. FEATURE_DIR must be absolute; treat each AVAILABLE_DOCS entry as a document name or path relative to FEATURE_DIR and resolve it against FEATURE_DIR before use. For single quotes in args like "I'm Groot", use escape syntax: e.g 'I'\''m Groot' (or double-quote if possible: "I'm Groot").
