...
**Status**: Landed in 4.1.0 (2026-09-10). All 52 tasks in `specs/016-molecule-install-hooks/tasks.md` are ticked; the last phases shipped in PRs #97 (Phase 7), #98 (Phase 8) and #99 (Phase 9 polish and version bump). Status as of 2026-09-09, kept for history: implementation in flight. Phases 1-6 landed on `main` as of 2026-09-09 (foundational schema/model/runner, US1 hook execution, US2 per-molecule failure policy, US3 `--no-install-hooks` opt-out, US4 multi-molecule ordering + hook-only molecules, PR #95). Remaining: Phase 7 (FR-025 hook-only transaction), Phase 8 (`spaex remove` WARN), Phase 9 (polish: `--help` docs, `README.md`, quickstart walk, schema-desc updates, backwards-compat validation, and the `pyproject.toml` bump to 4.1.0). See `specs/016-molecule-install-hooks/tasks.md` for the per-task status.
