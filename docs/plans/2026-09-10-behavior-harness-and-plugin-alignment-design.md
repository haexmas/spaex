...
**Implementation status (2026-09-11)**: [Spec 023](../../specs/023-behavior-harness/) Phases 1-3 (US1 MVP, PR [#101](https://github.com/haexmas/spaex/pull/101)) and Phase 5 (US3 hard-conflict paths, PR [#102](https://github.com/haexmas/spaex/pull/102)) are merged to main. Phase 6 (US4 clarification round-trip) lands on branch `023-phase-6-us4`, 627 tests pass. `spaex install` composes `.spaex.md` from every pinned molecule's behavior fragments, materializes `.spaex/constitution.d/`, verifies source/build-input hashes, and reuses the committed artifact when the hashes match. `spaex install --global [--dry-run|--check] [runtimes]` installs a per-runtime bootstrap block into `~/.claude/CLAUDE.md`, `$CODEX_HOME/AGENTS.md`, and every Gemini `context.fileName` target. Composer is CLI-only (priority `claude` → `codex` → `gemini`); the direct-API-via-litellm path from research.md §1 was superseded during implementation to avoid pulling a ~40-package adapter graph into every install, with a `stub_caller` seam retained for tests. On a Shape B response, `spaex.behavior.orchestrate.run` now answers each clarification question once (via stdin, refusing on a non-interactive TTY), persists the answer keyed by the cited fragments' current body hashes, and re-invokes the Composer once with it staged; a second Shape B response in the same build is an `invalid-output` failure. The existing reproducibility skip already avoids re-asking an unchanged fragment set, and the existing invalidation logic already re-asks once a cited fragment's body changes, so neither needed new code. Phase 4 (US2 authoring surface, T037-T038, test-only) remains the safest cut if scope needs trimming; Phase 7 (US5 project-local fragments) is next in priority order.

**Implementation status (2026-09-16)**: Dogfooding this repo's own `.spaex/manifest.json` (9 adopted molecules, ~60KB composer input) surfaced that the single-shot whole-fragment-set Composer call from Spec 023 does not scale: 4 of 5 real `spaex install` attempts timed out even at a 600s budget, and one returned a response that silently dropped 9 fragments (correctly caught and aborted by the existing FR-012b completeness check). Two hardening fixes landed directly on main: `composer.log` now survives the generation-rollback that previously erased it on failure (PR [#136](https://github.com/haexmas/spaex/pull/136)), and the Composer logs per-invocation progress/elapsed-time and appends host load averages to a timeout diagnostic (PR [#137](https://github.com/haexmas/spaex/pull/137)). The underlying scaling problem is specced separately as [Spec 026](../../specs/026-reliable-fragment-composition/) (PR [#138](https://github.com/haexmas/spaex/pull/138), spec/plan/tasks merged, implementation not yet started): a map-reduce composition replacing the single call with bounded batches plus a bounded merge/reduce tree, preserving byte-reproducibility, the sentinel/Shape A/Shape B contract, and the non-destructive/completeness guarantees.

**Implementation status (2026-09-21)**: Spec 026 shipped in 5.1.0 (PR [#140](https://github.com/haexmas/spaex/pull/140) and its follow-ups): deterministic fragment-to-batch partitioning, concurrent batch dispatch, per-batch completeness checks, and an independent merge ceiling that is a sanity backstop rather than a tuning knob ([ADRs 0022-0025](../adr/0022-map-reduce-composer.md)); `SPAEX_LLM_MODEL` now works on the CLI-only path, and the default Composer timeout was raised to 900s (PR [#153](https://github.com/haexmas/spaex/pull/153)). All 23 tasks of Spec 026 are ticked, including T020 against this project's real fragment set. Spec 023 is feature-complete with 77 of 78 tasks ticked, including project-local fragments (US5, PR [#104](https://github.com/haexmas/spaex/pull/104)) and the `spaex constitution build/trace` subcommands (Phase 10, PR [#113](https://github.com/haexmas/spaex/pull/113)); the one open task, T067, is a manual cross-runtime discoverability check that needs real Claude, Codex and Gemini sessions. [Spec 027](../../specs/027-generic-atom-delivery/) (5.1.0) extends the same install pipeline to non-behavior atom categories.
...
- [Composition UI & Skills Externalization Roadmap (2026-09-08)](2026-09-08-composition-ui-and-skills-externalization-roadmap.md): fixes the overall phasing (A: skills externalize; B: ontology cleanup, presets dropped 2026-09-21; C: `spaex status`; D: GUI; E: Creator flow). This design and spec 023 shift the phasing so behavior-harness lands as an additive 4.2.0 BEFORE the roadmap's Phase A (5.0.0).
...
- **Phase B (Ontology cleanup + presets, spec 019)**: grows to include the behavior-fragment machinery from this design. Concretely, Phase B now covers:
  ...
  - Named presets remain part of this phase. (2026-09-21: presets were dropped, see Decision 4 of the roadmap; the behavior-fragment machinery above shipped as Spec 023.)
- **Phase C (spaex status + provenance, spec 020)**: extended so `spaex status` shows the composed constitution's provenance: which fragment came from which atom, which molecule pulled the atom in. `spaex constitution trace <clause>` becomes a first-class command.
...
- [Spec 026 Reliable Fragment Composition (2026-09-16)](../../specs/026-reliable-fragment-composition/): map-reduce replacement for the single-shot Composer call this design introduced, born from a real scaling failure against this repo's own fragment set.
