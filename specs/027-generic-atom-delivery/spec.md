...
- What happens when the install transaction fails partway through writing generic-category files, or partway through regenerating a composed file? Existing install-transaction atomicity (already guaranteed for the whole `spaex install` run) applies unchanged — no new failure mode is introduced by this feature.
...
- **FR-009**: When any adopted molecule's `nix_packages` fragment is not a JSON array of non-empty strings, `spaex install` MUST refuse the entire install with a typed diagnostic naming the offending molecule id, rather than skipping that molecule's contribution and composing the rest. No composed-category file is written in that generation while the refusal stands.
...
- The composable dependency-contribution mechanism (FR-006a/FR-006b) is scoped to one fixed category for package/dependency fragments feeding a shared generated environment file — it reuses the existing behavior-fragment composition pipeline (Spec 023) rather than introducing a general "any category can opt into composable vs. exclusive" configuration knob; broadening that is left to future work if a concrete need arises.
...
- Existing install-transaction atomicity (Spec 008) already covers partial-failure rollback for the whole install run; this spec extends generic-category files (exclusive and composed) to participate in that same transaction rather than redefining atomicity.
