...
1. **Given** a molecule manifest declares files under a category other than `behavior`, `constitution` (the pre-Spec-023 legacy category, still separately handled by `spaex.constitution.resolve`/`publish`), `skill`, or `skills`, **When** the operator runs `spaex install`, **Then** each declared file is written at its repo-root-relative path with byte-identical content to the molecule source.
...
- **FR-001**: System MUST materialize, on `spaex install`, every file declared under an *exclusive* generic atom category (any category other than `behavior`, the pre-Spec-023 legacy `constitution` category, `skill`, `skills`, and the composable dependency-contribution category from FR-006a) verbatim to the repo-root-relative path given in the molecule manifest.
...
- **FR-008**: System MUST continue treating `behavior`, the pre-Spec-023 legacy `constitution` category, `skill`, and `skills` as reserved category names whose existing, specialized handling is unaffected by this generalization.
...
- **Exclusive Generic Atom Category**: A named group of files declared under a molecule's `atoms` map, for any category key other than `behavior`, the pre-Spec-023 legacy `constitution` category, `skill`, `skills`, or the composable dependency-contribution category, whose files are materialized verbatim to the consumer repo with a single owning molecule per path.
...
- The exclusive-delivery generalization covers any atom category name except the reserved `behavior`, the pre-Spec-023 legacy `constitution` category (still handled by its own, separate publication path — `spaex.constitution.resolve`/`publish` — unaffected by and unrelated to this feature despite superficially having "just a category key and declared paths" the same shape as a Spec 027 category), `skill`, `skills`, and composable dependency-contribution keys — no new category-naming vocabulary needs to be invented for the exclusive case; this matches the molecule-manifest schema's existing open `Dict[str, List[str]]` shape and the atoms publisher README's stated intent that environment-config files "can be declared under any category name a publisher chooses."
