...
- Reuses `invoke.py` verbatim for every batch call and the merge call: no changes to `_call_cli`, `_parse`, the sentinel format, or the five typed failure categories (`failure.py`). This satisfies FR-009 (no new silent-degradation surface) almost for free, since the existing machinery's guarantees carry over unchanged to every step.
...
**Rationale**: A contradiction between two fragments in *any* two batches is still caught as long as the top-level (root) reduce step sees the union of every batch's clauses with full provenance — which holds for both the flat N-ary merge and a pairwise tree, since a tree's root is, by construction, the union of every leaf. Building the merge primitive as a strict pairwise operation from the start (rather than retrofitting tree support later) means growing past the flat-merge threshold is an internal recursion change, not a contract change — Phase 1 in tasks.md can therefore ship the flat case only and treat pairwise recursion as a follow-up without redesigning the merge contract.
...
**Decision**: Replace the single `.spaex/composer.log` file with a small JSON-lines log: one line per internal step (`{"step": "batch-1", "raw_output": "...", "outcome": "..."}` style records), written incrementally as each step completes, at the same path (`$SPAEX_COMPOSER_LOG`, default `.spaex/composer.log`) so existing tooling/documentation pointing operators at that path keeps working. On any failure, every already-completed step's record is already on disk (written as it completed, not buffered until the end), so the existing `_GenerationRollback`-survives-the-log fix (already shipped) continues to apply per record rather than to one blob.
...
**Decision**: Determinism holds by construction as long as every step in the pipeline is itself deterministic given the same overall input: (a) fragment→batch assignment is a pure function of the canonically-sorted fragment list and the fixed size ceiling (§2) — no randomness, no wall-clock dependency; (b) the runtime is resolved once and fixed for the whole run (§4); (c) every batch/merge call uses the existing temperature-0 determinism aids already documented in `composer-interface.md`. No new source of nondeterminism is introduced.
