...
- A single fragment (or a small group that cannot reasonably be subdivided further) that is itself too large to compose reliably MUST still fail with a clear, typed diagnostic rather than being silently truncated or dropped.
...
- **FR-001**: The system MUST successfully compose a complete, correct constitution for a fragment set of any size a project adopts — there is no fixed ceiling on molecule or fragment count the system is allowed to stop scaling at. The project's current real-world scale (9 molecules, ~60KB combined fragment content) is the confirmed minimum bar, not the target ceiling.
...
- This feature is scoped to the Composer's reliability at scale. It does not cover unrelated composer concerns such as which CLI runtime is selected or how the effective prompt is authored.
