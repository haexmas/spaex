...
- [x] T020 Manually run `quickstart.md`'s walkthrough against this project's own real fragment set (currently 8 adopted molecules) once T001-T014 are implemented, confirming the actual observed behavior matches the documented example output. **Confirmed**: `spaex constitution build --force` against the main checkout's real fragment set (27 fragments across 8 adopted molecules, split into 2 batches + 1 merge) matched the quickstart's example shape exactly — per-batch/merge invoke/responded/composed progress lines, then `composed .spaex/constitution.md (27 fragment(s))`. This same real-world run is what surfaced and then confirmed the fix for the batch-level completeness gap (PR #143): earlier attempts against this fragment set silently dropped `javascript-typescript-testing`, `ponytail`, and `python-testing` at the batch step; after PR #143 the same batch cites all 27/27. `spaex install --speckit-agents claude,codex` then completed the full install, correctly hitting the reproducibility skip (`reused .spaex/constitution.md ... source_hash/build_input_hash unchanged`) and updating `.spaex/install.lock`.
