...
- [ ] T003 [P] Add a distinct "molecule path not found at this revision" error type to `src/spaex/util/errors.py` (name TBD by implementer — e.g. `MoleculeTreePathNotFoundError`; MUST be a different type from `MoleculeTreeExtractionError` per [contracts/get-or-extract.md](./contracts/get-or-extract.md)'s "distinguishable by type" requirement and FR-004). `diagnostic_key`, `exit_code`, and `hint` follow the same `HaexError` pattern.
...
- [ ] T009 [P] [US1] Add integration test `::test_repeated_request_is_a_cache_hit_no_reextraction` (AS2): call `get_or_extract` twice for the identical key; assert the same `Path` is returned both times and that no second `git archive` subprocess invocation occurs on the second call (spy/mock `subprocess.run`, matching quickstart.md section 2's pattern). Verifies FR-005, SC-002.
- [ ] T010 [P] [US1] Add integration test `::test_nonexistent_molecule_path_fails_distinctly` (AS3): request a `molecule_path` that doesn't exist at the given revision; assert the T003 not-found error type is raised (not `MoleculeTreeExtractionError`, not a generic exception) and that no directory exists at the would-be `final_dir` afterward. Verifies FR-004.
- [ ] T011 [P] [US1] Add integration test `::test_empty_but_tracked_path_succeeds_with_empty_directory` (Edge Case from spec.md): a molecule path that is declared and tracked in git but contains no files; assert materialization succeeds and returns an existing, empty directory (distinguishing this from the AS3 not-found case, per the empirically-verified `git archive` exit-code distinction in research.md). Verifies FR-004's "distinguishable from empty" requirement.
...
- [ ] T021 [P] [US4] Add a new test in `tests/unit/test_resolve.py` for FR-018's constitution-path escape rejection: a molecule manifest declares a constitution path that, once materialized, is actually a symlink pointing outside the materialized directory; assert resolution refuses it (raises `MoleculeTreeExtractionError`) rather than silently reading through the escape.
