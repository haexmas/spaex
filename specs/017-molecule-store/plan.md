...
**Storage**: Local filesystem cache under `SPAEX_STATE/molecule-store/<source-digest>/<revision>/<molecule-path>/`, sibling to the existing `SPAEX_STATE/repos/<digest>/` bare-clone cache. No new consumer-repo-visible files — nothing in this spec is copied into any consumer's `.spaex/manifest.json` or `.spaex/`.
