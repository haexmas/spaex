...
**Storage**: filesystem only. Fragments in `.spaex/constitution.d/<molecule-id>/<fragment-id>.md`. Composed constitution at `<repo-root>/.spaex/constitution.md`. Clarifications at `.spaex/clarifications.json` (new file, additive; format defined in `contracts/clarifications-schema.md`). Global bootstrap targets are resolved per runtime: Claude's fixed user file, Codex's `$CODEX_HOME` (default `~/.codex`), and Gemini's configured `~/.gemini/<context.fileName>` (paths per runtime; exact rules surveyed in research.md).
...
**Constraints**: Reproducibility contract requires `.spaex/constitution.md` be byte-identical from identical fragment sets (spec SC-003); Composer output is stabilized through system-prompt pinning + persisted clarifications, and the artifact is committed rather than regenerated on every install.
...
- **Principle I (No Secrets in Git)**: N/A. Behavior fragments and `.spaex/constitution.md` carry directives, not secrets. Global bootstrap writes to per-user config files that live outside any project repo. PASS.
- **Principle II (No Local Absolute Paths in Versioned Config)**: `.spaex/constitution.md` and `.spaex/constitution.d/` use repo-relative paths. Global bootstrap targets are per-user, per-OS files (`~/.claude/CLAUDE.md`, etc.) that are NOT under version control. No committed absolute paths introduced. PASS.
- **Principle III (Project Identity Is Device-Independent)**: `.spaex/constitution.md` is per-project, committed via git. Its content addresses no local paths. PASS.
...
- **Principle V (External Sources Are Opt-in Per Project)**: Behavior fragments arrive only through molecules explicitly listed in `.spaex/manifest.json`'s `compounds[]` allowlist. A project without pinned molecules produces an empty (or absent) `.spaex/constitution.md`. Project-local fragments (spec FR-018) are project-authored and thus not "external". PASS.
- **Policy-layer ownership**: `.spaex/constitution.md` is the generated spaex policy artifact and `.specify/memory/constitution.md` is the independent Spec-Kit policy artifact. `spaex install` regenerates only the spaex artifact; the developer's proposed git diff is the review gate. ADR-0018 records the separation and canonical directory layout. PASS.
...
│   ├── emit.py                   # Write .spaex/constitution.md at repo root; delete on empty set (per FR-017d)
...
│   ├── integration/
│   │   ├── test_install_end_to_end.py       # Fixture repos, full install → .spaex/constitution.md flow
...
│   │   ├── test_bootstrap_discoverability.py # SC-007 (mocked CLI-level runtime read of .spaex/constitution.md)
│   │   ├── test_clarification_persistence.py # SC-008 (answer once, no re-ask unchanged, re-ask on body change)
│   │   ├── test_add_time_plausibility.py    # FR-024a (add/remove warns, writes .stale, doesn't regen .spaex/constitution.md)
...
- **Canonical artifact**: `.spaex/constitution.md` is the single spaex policy output. Existing `atoms.constitution` and new behavior-fragment inputs converge there; the Spec-Kit constitution remains independent. Existing writers are consolidated in `src/spaex/behavior/emit.py` and `src/spaex/constitution/publish.py`.
