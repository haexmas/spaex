...
- Q: When and how hard does the semantic plausibility check run → A: Warn-not-block on `spaex add` and `spaex remove`. When the fragment set changes, the Composer runs a plausibility check that flags any cross-molecule semantic contradiction with provenance and marks the composition state as "stale, unresolved" via a sidecar file. The `add`/`remove` operation itself completes and its own file writes (`.spaex/manifest.json`, `install.lock`) proceed; `.spaex/constitution.md` is NOT regenerated at this stage. On the next `spaex install`, the stale flag requires operator reconciliation before a fresh `.spaex/constitution.md` is written (FR-010a). See FR-024a for the mandated behavior.
- Q: How does the composed constitution reach the agent → A: Two-layer emission. A one-time GLOBAL bootstrap resolves and writes a small spaex-managed block into the runtime's actual global instruction file: Claude uses `~/.claude/CLAUDE.md`; Codex uses `$CODEX_HOME/AGENTS.override.md` when that file is non-empty, otherwise `$CODEX_HOME/AGENTS.md`, with `CODEX_HOME` defaulting to `~/.codex`; Gemini uses every `~/.gemini/<context.fileName>` selected by `~/.gemini/settings.json`, defaulting to `GEMINI.md`. The block tells the agent "if `.spaex/constitution.md` exists in the current project root, read it and treat its content as project instructions". Per project, `spaex install` writes the composed constitution to a single file `.spaex/constitution.md` at the repo root; the entire file is spaex-owned. spaex NEVER modifies project-level `CLAUDE.md` or `AGENTS.md`.
...
**Independent Test**: given a fixture project with two pinned molecules that ship at least one behavior fragment each, running `spaex install` produces a `.spaex/constitution.md` at the repo root containing directives from both molecules with visible provenance. A runtime that has been through the global bootstrap and opens the project discovers `.spaex/constitution.md` without any project-level configuration.

**Acceptance Scenarios**:

1. **Given** a project pinning two molecules each shipping one behavior fragment, **When** the operator runs `spaex install`, **Then** `.spaex/constitution.md` exists at the repo root and lists both directives with a source label naming the originating molecule.
2. **Given** a fresh project with `.spaex/constitution.md` produced and a runtime that has run the global bootstrap, **When** the operator opens the runtime in the project, **Then** the runtime reads `.spaex/constitution.md` and treats its content as project instructions without any per-project configuration.
3. **Given** an operator switching agent runtime from Claude Code to Codex CLI in the same project, **When** they open the project without further configuration, **Then** the new runtime picks up the same behavior harness by reading the same `.spaex/constitution.md`.
...
Two developers pull the same commit of a project onto their machines. The project's `.spaex/manifest.json`, its `.spaex/constitution.d/` fragment set, and its root `.spaex/constitution.md` are all under version control. Both developers run `spaex install`. Neither developer's Composer re-runs because `.spaex/constitution.md` is up to date with the fragment set. Both developers' agent runtimes read the same `.spaex/constitution.md`. If a third developer changes a molecule pin (adding new fragments), the Composer runs on their machine, `.spaex/constitution.md` is regenerated, and they commit the updated file so the other two see the change through git pull rather than through re-computation.
...
**Independent Test**: given two clones of the same commit of a project with a committed `.spaex/constitution.md`, `spaex install` on both produces a byte-identical `.spaex/constitution.md` (or leaves the committed one untouched) and does not rerun the Composer.

**Acceptance Scenarios**:

1. **Given** a committed `.spaex/constitution.md` and unchanged fragments, **When** a fresh clone runs `spaex install`, **Then** the Composer is not invoked and `.spaex/constitution.md` is preserved unchanged.
2. **Given** a developer changes a molecule pin (adding new fragments), **When** they run `spaex install`, **Then** the Composer runs, `.spaex/constitution.md` is regenerated with a diff against the committed version, and the diff is stageable for commit.
3. **Given** `.spaex/constitution.md` and the fragment set drift out of sync (fragments changed on disk without a Composer run), **When** `spaex install` runs, **Then** the drift is detected and the Composer is invoked to regenerate `.spaex/constitution.md`.

---

### Edge Cases

- **Empty fragment set**: a project pinning molecules that ship zero behavior fragments produces either an empty `.spaex/constitution.md` or no `.spaex/constitution.md` at all (see FR-017d); the agent must not error when it looks for `.spaex/constitution.md` and does not find it.
...
- **FR-009**: The system MUST invoke the Composer only when the fragment set, effective Composer prompt content, Composer prompt version, or any valid persisted clarification answer has changed since the last successful build. It MUST compare both `source_hash` and `build_input_hash`, so downstream consumers who pull unchanged content reuse the committed artifact without incurring a build.
...
- **FR-016**: The bootstrap block MUST NOT embed project-specific content. It is static across all projects; per-project content lives exclusively in each project's `.spaex/constitution.md`.
- **FR-017**: The system MUST support installing or updating the bootstrap block in the resolved global instruction files of at least the three primary agent runtimes: Claude Code's fixed `~/.claude/CLAUDE.md`, Codex CLI's `CODEX_HOME` target with default `~/.codex` and its `AGENTS.override.md` precedence, and Gemini CLI's `~/.gemini/<context.fileName>` target(s) with default `GEMINI.md`. It MUST be extensible to additional runtimes without breaking these three.

**Per-project emission** (every `spaex install`)

- **FR-017a**: The system MUST write the composed constitution to a single file `.spaex/constitution.md` at the consumer's repo root.
- **FR-017b**: The entire `.spaex/constitution.md` file is spaex-owned. The system MAY overwrite it in full on every install; there is no delimited section within `.spaex/constitution.md`.
...
- **FR-017d**: When a project has no active behavior fragments (empty fragment set), the system MAY produce an empty `.spaex/constitution.md` or omit the file; both behaviors MUST NOT cause an agent to error.

**Project overrides**

- **FR-018**: The system MUST allow the project to contribute additive behavior fragments alongside molecule pins, either inline in `.spaex/manifest.json` or as file references to project-local fragment files.
...
- **FR-023**: After the global bootstrap is installed for a given runtime (per FR-014), the system MUST guarantee that the runtime picks up any project's `.spaex/constitution.md` without further per-project configuration by the consumer. At minimum this applies to Claude Code's fixed target, Codex CLI's resolved `CODEX_HOME` target, and Gemini CLI's resolved `context.fileName` target(s). Additional runtimes (dsh, and others) MAY be added by extending the bootstrap to their global instruction files without breaking these three.
- **FR-024**: The per-project `.spaex/constitution.md` MUST be identical across runtimes; the multi-agent-portability contract is that the same composed constitution reaches every runtime that has run the global bootstrap.
- **FR-024a**: The system MUST run the Composer plausibility check whenever `spaex add` or `spaex remove` changes the active fragment set. On detected cross-molecule semantic contradiction, the system MUST print a WARN identifying the two involved fragments and their source molecules, mark the composition state as "stale, unresolved" via a sidecar file under `.spaex/`, and complete the `add`/`remove` operation without abort. The system MUST NOT regenerate `.spaex/constitution.md` from this check; that regeneration is deferred to the next `spaex install`, which then requires reconciliation before writing per FR-010a. See Clarification Q2.
...
- **Project-local fragment**: a fragment contributed by the project itself (not by any pinned molecule), stored inline in `.spaex/manifest.json` or as a project-controlled file. Additive only.
- **Per-project constitution file (`.spaex/constitution.md`)**: the single spaex-owned file at the consumer's repo root that contains the composed constitution. The entire file is spaex-managed; there is no delimited section within it.
- **Global bootstrap block**: the small, static spaex-managed section inside the runtime-resolved user-global instruction file (Claude Code's fixed `~/.claude/CLAUDE.md`, Codex CLI's `CODEX_HOME` target, or Gemini CLI's configured `~/.gemini/<context.fileName>` target) that tells the agent runtime to look for and read `.spaex/constitution.md` from the current project root. Installed once per user per runtime, upgradeable across spaex versions via the delimited section.
...
- **SC-005**: A project maintainer adds a local additive fragment through `.spaex/manifest.json` (inline or file reference) without editing any molecule source, and it appears in the composed constitution on the next install.
...
- **SC-007**: A consumer who has run the global bootstrap for each of the three primary agent runtimes (Claude Code, Codex CLI, Gemini CLI) and then switches between them on the same project observes the same behavior harness without editing any file, because every runtime reads the same `.spaex/constitution.md`.
...
- **SC-011**: When the Composer fails for any reason (timeout, runtime error, malformed output, quota exceeded), `spaex install` exits non-zero with a diagnostic naming the failure category, and no existing `.spaex/constitution.md` is modified. Verified by fault-injection tests covering each failure category.
...
- **Global bootstrap is per-user, per-runtime, opt-in**: the consumer runs a one-time `spaex install --global` (or equivalent, exact CLI shape settled in the follow-up implementation spec) to install the bootstrap block into each agent runtime's global instruction file. Without the bootstrap, the runtime does not automatically read `.spaex/constitution.md`. The system surfaces this state at install time (see the "Bootstrap not yet installed" edge case).
