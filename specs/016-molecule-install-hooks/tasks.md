...
- [ ] T009 Extend `src/spaex/constitution/resolve.py::resolve_constitution_contributions` (or replace it with an equivalent result type) so the install path receives the complete resolved-molecule collection, not only constitution contributions: (a) add `repo_dir: Path`, `molecule_path: str`, `install_hook: InstallHook | None`, and `effective_priority: int` fields to `ResolvedMolecule` (per data-model.md's 2026-09-09 amendment — `repo_dir`/`molecule_path` replace the earlier, incorrect assumption of a pre-populated `cache_dir`; Spec 017's `molecule_store.get_or_extract()` lands 2026-09-08/09 and is what actually materializes a molecule's tree, on demand, from these two fields plus `source_url`/`revision`); (b) compute `effective_priority` as the consumer's `compounds[].config[<molecule-id>].priority` override if present, else the publisher's `manifest.priority`; (c) ensure the resolver emits a `ResolvedMolecule` for EVERY selected molecule, including molecules whose manifest declares NO `atoms.constitution` — this resolver already computes `repo_dir` (from `clone_dir()`) and `publisher_entry.path` per iteration; expose both on the record instead of discarding them once the constitution-body read is done. Verifies FR-007, FR-008.
- [ ] T010 [P] Add unit tests in `tests/unit/test_resolver_hook_only_molecule.py`: resolver returns record for hook-only molecule (no atoms.constitution); resolver returns install_hook on the record; effective_priority reflects consumer override when present. Verifies FR-007, FR-008.

### Path containment helper

- [x] ~~T011 [P] Create `src/spaex/util/path_containment.py` exposing `canonicalise_within(...)`...~~ **SUPERSEDED by Spec 017 (merged 2026-09-09).** Spec 017's `molecule_store._validate_and_extract()` already validates every file and symlink inside a materialized molecule tree against escapes (`..`-paths, absolute/relative symlink targets) at MATERIALIZATION time, for the whole tree — before the hook runner (T013) ever receives `molecule_dir`. `install_hook.script` is separately validated as a safe `RepoRelativePath` string at manifest-parse time (existing, unrelated to this task). Building a second, bespoke containment checker in T013 for a path that already lives inside an already-validated tree would duplicate Spec 017's guarantee, not add a new one. Do NOT implement this task; verified obsolete during the 2026-09-09 amendment. Verifies FR-014, FR-015 (now satisfied transitively via Spec 017's own contract).
- [x] ~~T012 [P] Add unit tests in `tests/unit/test_path_containment.py`...~~ **SUPERSEDED alongside T011** — no bespoke module to test. Spec 017's own `tests/unit/test_molecule_store_safety.py` (12 tests, merged 2026-09-09) already covers the equivalent escape scenarios at the point where they actually matter (extraction time).

### Hook runner

- [ ] T013 Create `src/spaex/install/hook_runner.py` exposing `run_install_hook(resolved: ResolvedMolecule, consumer_repo_root: Path, state_root: Path) -> HookOutcome`: (a) call `shutil.which(resolved.install_hook.interpreter)` — missing → `HookOutcome.LAUNCH_FAILURE` with reason `interpreter_not_on_path`, WITHOUT calling `get_or_extract()` first (no point materializing a tree for a hook that cannot run anyway); (b) call `spaex.git.molecule_store.get_or_extract(resolved.repo_dir, resolved.source_url, resolved.revision, resolved.molecule_path, state_root)` (Spec 017, merged 2026-09-09) to obtain `molecule_dir` — a `MoleculeTreePathNotFoundError` or `MoleculeTreeExtractionError` from that call → `HookOutcome.LAUNCH_FAILURE` with reason `molecule_tree_unavailable` (Spec 017's own path-containment validation already covers the escape checks this task's earlier draft assigned to a bespoke `canonicalise_within`/`PathEscapeError` pair — do NOT reimplement that logic here, it is Spec 017's responsibility); (c) build target = `molecule_dir / resolved.install_hook.script` (already validated safe by construction, since `molecule_dir`'s entire content passed Spec 017's `_validate_and_extract`, and `install_hook.script` was validated as a `RepoRelativePath` at manifest-parse time — no further containment check needed here); (d) build argv `[interpreter, str(target), *install_hook.args]`; (e) call `subprocess.run(argv, cwd=consumer_repo_root, check=False)` with NO `capture_output=True`, NO `env=` override (inherits complete env per FR-013 clarification), and NO `stdin=` override; (f) on `OSError` → `HookOutcome.LAUNCH_FAILURE` reason `process_launch_oserror`; (g) catch `KeyboardInterrupt` raised while waiting for `subprocess.run` and return `HookOutcome.INTERRUPTED` with reason `interrupted`, so the configured policy is applied; (h) on returncode != 0 → `HookOutcome.NONZERO_EXIT` with the code; (i) on returncode == 0 → `HookOutcome.OK`. Verifies FR-009 (partially: runner is called after atoms), FR-012, FR-013 (env + stdio), FR-014, FR-015.
- [ ] T014 [P] Add unit tests in `tests/unit/test_hook_runner.py`: subprocess called with correct argv, explicit consumer-root `cwd`, and no env override; missing interpreter → LAUNCH_FAILURE/interpreter_not_on_path, asserted via a mock/spy that `molecule_store.get_or_extract` is NOT called in this case (no point materializing for a hook that cannot run); `get_or_extract` raising `MoleculeTreePathNotFoundError` or `MoleculeTreeExtractionError` (mock it directly — this task does not need a real git repo) → `HookOutcome.LAUNCH_FAILURE` reason `molecule_tree_unavailable`; simulated `subprocess.run` `OSError` → `HookOutcome.LAUNCH_FAILURE` reason `process_launch_oserror`; simulated `KeyboardInterrupt` → `HookOutcome.INTERRUPTED` with reason `interrupted`; nonzero exit → NONZERO_EXIT with code; zero exit → OK, with the built argv's script path verified to be `<mocked-molecule_dir>/<install_hook.script>`. Verifies FR-012, FR-013, FR-014, FR-015.
...
- [ ] T017 [US1] Wire hook execution into `src/spaex/cli/install.py::run`: update the resolver result consumed by `run` to include ALL resolved molecules (including hook-only molecules), remove or replace the existing single-constitution-molecule guard, and after ALL atoms materialise into the Spec-008 staging generation iterate the collection in ascending `effective_priority` then ascending UTF-8 molecule-ID. For each record with `install_hook != None`, call `hook_runner.run_install_hook(record, consumer_repo_root=repo_root)` and collect outcomes into a per-molecule status map. Do NOT invoke `publish_constitution()` (or its Spec-008 generalisation) until hook execution completes. Verifies FR-009, FR-010, FR-011.
- [ ] T018 [US1] Translate `HookOutcome` values into `hook_status` field values written to install.lock (via T015 writer). For US1 default policy (`on_failure="abort"`), OK → `hook_status: "ok"`. Non-OK outcomes are handled in US2 phase. Verifies FR-021.

### Integration tests

- [ ] T019 [P] [US1] Add integration test in `tests/integration/test_install_hook_execution.py::test_hook_runs_after_atoms_and_writes_marker` (AS1): fixture publisher molecule with install_hook script appending `graphify-out/` to `.gitignore`; consumer runs `spaex add`; assert `.gitignore` contains line exactly once, `.spaex/constitution.md` present, `.spaex/install.lock` has `hook_status: "ok"`. Verifies FR-009, FR-021.
- [ ] T020 [P] [US1] Add integration test `::test_hook_is_idempotent_on_second_install` (AS2): run install twice; assert `.gitignore` line still exactly once (no duplicate), hook_status stays `"ok"`. Verifies FR-024.
- [ ] T021 [P] [US1] Add integration test `::test_hook_interactive_prompt_via_pty` (AS3): use `pexpect` or `pty` fixture to attach a pseudo-TTY; hook script calls `input()`; assert prompt appears, typed answer reaches the hook. Skip on Windows CI if needed. Verifies FR-013 stdio-inheritance in TTY.
- [ ] T022 [P] [US1] Add integration test `::test_hook_eoferror_in_non_tty_env` (edge case documented in spec § Edge Cases): stdin piped from `/dev/null`; hook calls `input()`; expect EOFError; molecule-author's fallback (default_yes) proves flow completes. Verifies interactive-prompt Assumption.
...
- [ ] T035 [US4] Verify hook orchestration (T017) uses the exact sort key `(effective_priority, molecule_id.encode("utf-8"))` ascending. If T017 already uses this — no code change; add a comment referencing FR-010 for future readers. Verifies FR-010.
- [ ] T036 [P] [US4] Add integration test in `tests/integration/test_install_hook_multi_molecule.py::test_priority_order_execution` (AS1): fixture two molecules, priorities 10 and 20; hooks each append `<molecule_id>\n` to `<consumer>/hook-log.txt`; run install; assert log's line order is molecule-with-priority-10 before molecule-with-priority-20. Verifies FR-010.
- [ ] T037 [P] [US4] Add integration test `::test_hook_only_molecule_hook_runs` (AS2): fixture molecule whose manifest declares `install_hook` but has NO `atoms.constitution`; adopt; assert hook ran (marker file present), install.lock has a record for this molecule with `hook_status: "ok"`. Verifies FR-007.
- [ ] T038 [P] [US4] Add integration test `::test_abort_stops_later_hooks_total_rollback` (AS3): fixture three molecules with hooks; the middle one (by priority) has `on_failure="abort"` and exits 1; adopt all three; assert the third molecule's hook did NOT run, ALL atoms rolled back (no `.spaex/` writes), `.spaex.json` compound entries NOT added. Verifies FR-016, FR-025 (integration of rollback with multi-molecule ordering).
...
- **Path-containment track**: T011/T012 SUPERSEDED by Spec 017 (2026-09-09 amendment) — no longer a dependency of anything below.
- **Hook-runner track**: T013 → T014. Depends on T006 (dataclass) and Spec 017's `molecule_store.get_or_extract()` (merged 2026-09-09, no longer an in-repo task this spec needs to build). Once T006 lands, parallel to install.lock track (T015-T016).
