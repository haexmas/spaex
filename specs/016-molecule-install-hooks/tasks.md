...
- [ ] T009 Extend `src/spaex/constitution/resolve.py::resolve_constitution_contributions` (or replace it with an equivalent result type) so the install path receives the complete resolved-molecule collection, not only constitution contributions: (a) add `install_hook: InstallHook | None` and `effective_priority: int` fields to `ResolvedMolecule`; (b) compute `effective_priority` as the consumer's `compounds[].config[<molecule-id>].priority` override if present, else the publisher's `manifest.priority`; (c) ensure the resolver emits a `ResolvedMolecule` for EVERY selected molecule, including molecules whose manifest declares NO `atoms.constitution`. Verifies FR-007, FR-008.
...
- [ ] T013 Create `src/spaex/install/hook_runner.py` exposing `run_install_hook(resolved: ResolvedMolecule, consumer_repo_root: Path) -> HookOutcome`: (a) call `shutil.which(resolved.install_hook.interpreter)` — missing → `HookOutcome.LAUNCH_FAILURE` with reason `interpreter_not_on_path`; (b) build target = `canonicalise_within(resolved.cache_dir, resolved.cache_dir / resolved.install_hook.script)` — `PathEscapeError` → `HookOutcome.LAUNCH_FAILURE` with reason `path_containment_failure`; (c) build argv `[interpreter, str(target), *install_hook.args]`; (d) call `subprocess.run(argv, cwd=consumer_repo_root, check=False)` with NO `capture_output=True`, NO `env=` override (inherits complete env per FR-013 clarification), and NO `stdin=` override; (e) on `OSError` → `HookOutcome.LAUNCH_FAILURE` reason `process_launch_oserror`; (f) catch `KeyboardInterrupt` raised while waiting for `subprocess.run` and return `HookOutcome.INTERRUPTED` with reason `interrupted`, so the configured policy is applied; (g) on returncode != 0 → `HookOutcome.NONZERO_EXIT` with the code; (h) on returncode == 0 → `HookOutcome.OK`. Verifies FR-009 (partially: runner is called after atoms), FR-012, FR-013 (env + stdio), FR-014, FR-015.
- [ ] T014 [P] Add unit tests in `tests/unit/test_hook_runner.py`: subprocess called with correct argv, explicit consumer-root `cwd`, and no env override; missing interpreter → LAUNCH_FAILURE/interpreter_not_on_path; script outside cache → LAUNCH_FAILURE/path_containment_failure; simulated OSError → LAUNCH_FAILURE/process_launch_oserror; simulated `KeyboardInterrupt` → `HookOutcome.INTERRUPTED` with reason `interrupted`; nonzero exit → NONZERO_EXIT with code; zero exit → OK. Verifies FR-012, FR-013, FR-014, FR-015.
