...
- [ ] T009 Extend the resolver in `src/spaex/install/__init__.py` (or wherever `ResolvedMolecule` and the resolver live today; verify via `grep -rn "ResolvedMolecule\|def.*resolve" src/spaex/install/`): (a) add `install_hook: InstallHook | None` and `effective_priority: int` fields to `ResolvedMolecule`; (b) compute `effective_priority` as consumer's `compounds[].config[<molecule-id>].priority` override if present, else publisher's `manifest.priority`; (c) ensure the resolver emits a `ResolvedMolecule` for EVERY selected molecule, including molecules whose manifest declares NO `atoms.constitution`. Verifies FR-007, FR-008.
...
- [ ] T013 Create `src/spaex/install/hook_runner.py` exposing `run_install_hook(resolved: ResolvedMolecule) -> HookOutcome`: (a) call `shutil.which(resolved.install_hook.interpreter)` — missing → `HookOutcome.LAUNCH_FAILURE` with reason `interpreter_not_on_path`; (b) build target = `canonicalise_within(resolved.cache_dir, resolved.cache_dir / resolved.install_hook.script)` — `PathEscapeError` → `HookOutcome.LAUNCH_FAILURE` with reason `path_containment_failure`; (c) build argv `[interpreter, str(target), *install_hook.args]`; (d) `subprocess.run(argv, cwd=consumer_repo_root, check=False)` with NO `capture_output=True`, NO `env=` override (inherits complete env per FR-013 clarification), NO `stdin=` override; (e) on `OSError` → `HookOutcome.LAUNCH_FAILURE` reason `process_launch_oserror`; (f) on returncode != 0 → `HookOutcome.NONZERO_EXIT` with the code; (g) on returncode == 0 → `HookOutcome.OK`. Verifies FR-009 (partially: runner is called after atoms), FR-012, FR-013 (env + stdio), FR-014, FR-015.
- [ ] T014 [P] Add unit tests in `tests/unit/test_hook_runner.py`: subprocess called with correct argv, cwd, no env override; missing interpreter → LAUNCH_FAILURE/interpreter_not_on_path; script outside cache → LAUNCH_FAILURE/path_containment_failure; simulated OSError → LAUNCH_FAILURE/process_launch_oserror; nonzero exit → NONZERO_EXIT with code; zero exit → OK. Verifies FR-012, FR-013, FR-014, FR-015.

### install.lock writer/reader

- [ ] T015 Extend install.lock writer in `src/spaex/install/__init__.py` (or the module that composes install.lock; grep for `install.lock`, `install_lock`, `MoleculeLockRecord`): serialise optional `hook_status: Literal["ok", "failed", "skipped"] | None` on each per-molecule record. Emit the field ONLY when the molecule declares `install_hook` in the pinned revision. Verifies FR-020 through FR-023, data-model.md § install.lock per-molecule record.
- [ ] T016 [P] Extend install.lock reader (same module as T015) to parse the optional `hook_status` field: unknown value → schema-validation error via existing lock schema check; absent → None on the parsed record. Verifies backwards-compat SC-006.
...
- [ ] T017 [US1] Wire hook execution into `spaex install` orchestration in `src/spaex/install/__init__.py`: after ALL atoms materialise into the Spec-008 staging generation, iterate `ResolvedMolecule` collection in ascending `effective_priority` then ascending UTF-8 molecule-ID; for each record with `install_hook != None`, call `hook_runner.run_install_hook(record)`; collect outcomes into a per-molecule status map. Do NOT invoke `publish_constitution()` (or its Spec-008 generalisation) until hook execution completes. Verifies FR-009, FR-010, FR-011.
...
- [ ] T025 [P] [US2] Add integration test in `tests/integration/test_install_hook_failure_policy.py::test_abort_rolls_back_transaction` (AS1): fixture molecule with `on_failure="abort"` and script that exits 1; `spaex add`; assert `.spaex/` directory unchanged from pre-add state (byte-for-byte compare); `.spaex.json` compound-entry NOT added; CLI exits non-zero; diagnostic key is `install-failed` with molecule id in context. Verifies FR-016, FR-017, SC-003.
...
- [ ] T039 In install orchestration: after hooks run, compare the newly-computed hook_status map against the current install.lock's hook_status map. If atom bytes are unchanged AND hook_status map is unchanged → clean no-op (do NOT publish a new generation). If atom bytes are unchanged BUT hook_status map differs → publish a new install.lock generation containing only the hook_status delta (no atom file rewrites). If atom bytes changed → normal generation publish (regardless of hook_status). Verifies FR-025.
- [ ] T040 [P] Add integration test in `tests/integration/test_install_hook_idempotency.py::test_hook_only_transaction_publishes_new_generation`: run `spaex install` once (hook_status=ok); mutate the molecule cache so the hook now exits 1 with `on_failure=warn`; run `spaex install` again (atom bytes unchanged; hook_status flips to failed); assert new install.lock generation was published with the changed hook_status. Verifies FR-025.
...
- **Model track**: T006 (dataclass) → T007 (parser) → T008 (parser tests). Sequential within track; parallel to schema track and path-containment track.
- **Path-containment track**: T011 → T012. Parallel to schema and model tracks.
- **Hook-runner track**: T013 → T014. Depends on T006 (dataclass) and T011 (path-containment helper). Once those land, parallel to install.lock track (T015-T016).
