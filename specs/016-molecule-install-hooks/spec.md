...
- **Spec 017** (Molecule tree materialization store): **hard blocking dependency**, discovered mid-implementation of this spec (2026-09-08) and landed 2026-09-09 across three merged PRs. Provides `spaex.git.molecule_store.get_or_extract()`, the only way this spec's hook_runner can obtain a real, on-disk directory to hand a script to a subprocess interpreter — no such capability existed anywhere in spaex before Spec 017. Also supplies the path-containment validation this spec originally planned to build itself (a bespoke `util/path_containment.py`, since superseded — see plan.md's 2026-09-09 amendment).
