...
**Independent Test**: publish two molecules, one with `on_failure: "abort"` and one with `on_failure: "warn"`, both pointing at scripts that exit non-zero. Verify `spaex add` of the abort-molecule rolls back all spaex-managed writes while external hook side effects, if any, are not promised to roll back; verify `spaex add` of the warn-molecule completes with `hook_status: "failed"` in install.lock and exit code 0.
...
- **FR-008**: Each `ResolvedMolecule` record MUST expose the parsed `install_hook`, canonical source URL, pinned lowercase 40-hex revision, local bare-clone `repo_dir`, publisher-declared `molecule_path`, and effective priority (consumer override if present, publisher default otherwise). These inputs let the hook runner obtain the molecule directory lazily through Spec 017's `get_or_extract()`; constructing the record MUST NOT require eager extraction. `repo_dir` is a local execution input, never a cache identity or versioned cross-device reference.
...
- **ResolvedMolecule**: complete molecule-level resolver record introduced for this feature, retaining parsed hook, publisher source URL, pinned revision, local bare-clone `repo_dir`, publisher-declared `molecule_path`, and effective priority. Hook execution consumes this record without re-reading the raw manifest. Existing constitution contributions remain available for publication.
...
- **The molecule store is keyed by source, pinned SHA, and molecule path**: Spec 017's landed `get_or_extract()` API materializes the selected subtree lazily from the existing bare clone. It does not fetch publishers or verify cached content integrity. Execution-time script containment (FR-014) remains necessary even on a cache hit; archive validation alone does not establish the script's molecule-directory boundary.
