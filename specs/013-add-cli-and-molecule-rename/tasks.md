...
- [X] T044 [P] [US2] Add an integration test in `tests/integration/test_migrate_end_to_end.py`: on a fixture v2 repo (consumer + publisher + molecule), run `haex migrate` and assert each expected `.migrated` sibling exists with content matching the v3 schema.
- [ ] T044R [US2] (deferred) Add remote immutable-SHA publisher proposal coverage: assert the proposal lands under `$HAEX_HIVE_STATE/migrations/<source-digest>/<revision>/manifest.json.migrated`.
...
- [X] T054 [US2] Implement local proposal placement per [contracts/haex-migrate.v2-to-v3.md](./contracts/haex-migrate.v2-to-v3.md): `.haex-hive.json.migrated` sibling, `manifest.json.migrated` sibling, and `<molecule-dir>/manifest.json.migrated` sibling.
- [ ] T054R [US2] (deferred) Implement remote proposal placement under `$HAEX_HIVE_STATE/migrations/<source-digest>/<revision>/<repo-relative-path>.migrated`; reuse the `clone_dir()` digest helper from `src/haex_hive/migrate/transform.py`.
...
- [X] T090 [P] Assert byte-identical `.haex-hive/install.lock` output between two consecutive `haex install` runs on unchanged inputs (Spec 008 SC-003 preserved). Codified in [tests/integration/test_install_lock_byte_identical.py](../../tests/integration/test_install_lock_byte_identical.py). `.haex-hive/visibility.json` dropped from the original wording, retired by PR 67's 2026-09-03 install.lock npm/pip-shape amendment (`install.lock` is now the sole publication record).
- [X] T091 [P] Add an end-to-end test in `tests/integration/test_atoms_repo_installation.py`: `haex add https://github.com/haexmas/atoms com.github.haexmas.atoms.graphify-first-authoring` against a fixture consumer, then `haex install`, then assert the graphify constitution.md and its accompanying files land under `.haex-hive/` per Spec 007 v3 plus Spec 008. This closes SC-001 (the atoms-repo molecule installs into a v3-adopted consumer through a single call).
- [X] T092 [P] Regenerate the [quickstart.md](./quickstart.md) walkthrough against the final CLI by running each command block and confirming outputs match the documented expectations; update the doc for any ergonomic drift.
- [X] T093 [P] Grep for lingering v2 vocabulary across `docs/`, `specs/`, and `src/haex_hive/`: search for `atom-manifest.v2`, `haex-hive.v2`, `contributes:` (in prose about the current tool, not historical docs), and `includes[]` in prose. Replace with v3 equivalents where the reference is about current-tool behavior. Historical spec documents (Specs 001-011) stay unchanged; they describe what was landed at the time. **Finding**: every current-tool reference to v2 vocabulary in `src/haex_hive/` is either (a) legitimate `migrate/` code that operates on v2 inputs by design, (b) schema `description` strings and dataclass docstrings that document the v3→v2 rename as onboarding help, or (c) the dormant `NoSourcesDeclaredError.hint` in `util/errors.py` (dead prose after PR 73 removed the raise; goes with the class if it is ever retired). Every reference under `docs/plans/` is a historical design preview covered by the task's exemption. No replacements required.
- [X] T094 Update memory `haex_hive_pre_user` in the operator's memory index to note that Spec 013 is landed and v3 is the tool's baseline. Follow the existing memory-file format; no code impact. Updated 2026-09-06; the memory now also carries the PR 70 review-lesson on validating operator-supplied paths (`RepoRelativePath.validate` + `resolve().relative_to(root)`) and JSON container shapes before use. Not code-tracked; landed in the operator's local memory index.
- [X] T095 Update `docs/plans/2026-09-02-spec-013-add-cli-and-molecule-rename-design.md` header from `Status: Design preview` to `Status: Landed as Spec 013 on 2026-09-06` with a pointer to `specs/013-add-cli-and-molecule-rename/`.
