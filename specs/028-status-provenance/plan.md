...
**Performance Goals**: SC-001 — under 5 seconds for `spaex status` on this repository's 9 molecules. Both commands are in-process filesystem reads and string/JSON parsing on small inputs (single-digit-to-low-tens of molecules, a few dozen fragment files); no scaling concern comparable to the `behavior` Composer's LLM calls.
