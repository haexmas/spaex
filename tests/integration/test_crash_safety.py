...
`spaex install` cleans any leftover `<root>.next/` / `<root>.prev/` siblings
and converges on the idempotent state — the 2026-09-02 detect+retry model
that replaced the earlier 8-state recovery-forward dispatcher.
...
_CRASH_CASES = ["pre_swap", "rename_a", "rename_b"]
...
def test_crash_at_boundary_converges_on_retry(
    single_source_constitution_fixture: dict, crash_point: str
) -> None:
    ...
    if crash_point == "rename_a":
        previous_generation = (prev_dir / "constitution.md").read_bytes()
        manifest_path = prev_dir / "manifest.json"
        ...
        (live / "manifest.json").write_bytes(manifest_bytes)
